...
to_process = {
    ...
    'NN': {
        'path': 'NN_OUTPUT/CreditCardFraud',    # CreditCardFraud, WiscosinBreastCancerData
        'nn_curve': True,
...
def plot_data(title, data, column_prefixes=None, validate_only=False, nn_curve=False, clear_existing=True, ylim=None,
              x_scale='linear', y_scale='linear', legend_name=None, x_label='Iterations (count)', y_label='Fitness'):
    ...
    if column_prefixes is not None:
        for i, column_prefix in enumerate(column_prefixes):
            if not nn_curve:
                ...
                plt.plot(data.index, mean, '-', linewidth=1, markersize=1, label=column_prefix)
            else:
                train = data['{}_f1_trg'.format(column_prefix)]
                test = data['{}_f1_tst'.format(column_prefix)]
                val = data['{}_f1_val'.format(column_prefix)]
                if not validate_only:
                    plt.plot(data.index, train, '-', linewidth=2, markersize=1, label='Train {} {}'.format(column_prefix, y_label))
                    plt.plot(data.index, val, '-', linewidth=2, markersize=1, label='CV {} {}'.format(column_prefix, y_label))
                else:
                    plt.plot(data.index, test, '-', linewidth=2, markersize=1, label='{} {}'.format(column_prefix, y_label))
        ...
        if not nn_curve:
            ...
            std = data['std']
            # plt.fill_between(data.index, mean - std, mean + std, alpha=0.2)
            plt.plot(data.index, mean, '-', linewidth=2, markersize=1, label=None)
        else:
            train = data['f1_trg']
            val = data['f1_val']
            plt.plot(data.index, train, '-', linewidth=2, markersize=1, label='Train {}'.format(y_label))
            plt.plot(data.index, val, '-', linewidth=2, markersize=1, label='CV {}'.format(y_label))

    plt.legend(title=legend_name, loc="best", fontsize=15)
    return plt
    ...
    for samples in mimic_files.keys():
        ...
        for keep in mimic_files[samples].keys():
            ...
            if nn_curve:
                ...
                p = plot_data('{} - MIMIC {} {}: {} vs Iterations'.format(problem_name, samples, keep, y.capitalize()), main_df[y],
                ...
                p.savefig('{}/{}/MIMIC_{}_{}_{}.png'.format(output_dir, problem_name, samples, keep, y.capitalize()), format='png', dpi=150)
            else:
                for y in graph_ys:
                    p = plot_data('{} - MIMIC {} {}: {} vs Iterations'.format(problem_name, samples, keep, y.capitalize()), main_df[y],
                                  sorted(mimic_files[samples][keep].keys()),
...
def plot_best_curves(problem_name, files, output_dir, nn_curve=False):
    ...
    if nn_curve:
        ...
        p = plot_data('{} - Best: {} vs Iterations'.format(problem_name, 'Function Evals'), main_df['fevals'],
                      prefixes, nn_curve=nn_curve, validate_only=nn_curve, y_label=y_label)
        p.savefig('{}/{}/Best_{}.png'.format(output_dir, problem_name, 'Fevals'), format='png', dpi=200)

        p = plot_data('{} - Best: {} vs Iterations'.format(problem_name, 'Clock Time'), main_df['time'],
                      prefixes, y_scale="log", nn_curve=nn_curve, validate_only=nn_curve, y_label="Log-scale Clock Time (seconds)")
        p.savefig('{}/{}/Best_{}.png'.format(output_dir, problem_name, 'Clock_Time'), format='png', dpi=200)

        main_df = main_df['fitness']

    p = plot_data('{} - Best: {} vs Iterations'.format(problem_name, y_label), main_df, prefixes,
                  nn_curve=nn_curve, validate_only=nn_curve, y_label=y_label)
    ...
    with open(input_path + '/best_results.csv', 'w+') as f:
        ...
        for problem_name in sorted(the_best):
            ...
            for algo in best_files:
                ...
                f.write('{},{},"{}",{},{},{},{}\n'.format(problem_name, algo, params, best_value, best_iterations, best_time, best_fevals))
